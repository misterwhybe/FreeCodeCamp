...
def calculate_demographic_data(print_data=True):
    ...
    high_income_education = df[df['education'].isin(advanced_education) &     (df['salary'] == '>50K')]
    low_income_education = df[~df['education'].isin(advanced_education)       &     (df['salary'] == '>50K')]
    # Calculate the percentage of people with advanced education who make more than 50K
    higher_education_rich = round((len(high_income_education) /           
    len(df[df['education'].isin(advanced_education)])) * 100, 1)
    lower_education_rich = round((len(low_income_education) /           
    len(df[~df['education'].isin(advanced_education)])) * 100, 1)    
    ...
    country_percentages = df[df['salary'] == '>50K'].groupby('native-country').size() / df.groupby('native-country').size() * 100
    ...
    india_high_income = df[(df['native-country'] == 'India') &            
    (df['salary'] == '>50K')]

    top_IN_occupation = india_high_income['occupation'].mode().values[0]
